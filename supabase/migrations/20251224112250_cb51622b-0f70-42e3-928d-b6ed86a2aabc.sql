...
ALTER TABLE public.profiles ADD COLUMN occupation text[] DEFAULT NULL;
...
BEGIN
  ...
  VALUES (
    ...
    CASE 
      ...
      ELSE NULL 
    END,
    NEW.raw_user_meta_data->>'specific_experience',
    CASE 
      WHEN NEW.raw_user_meta_data->'preferred_languages' IS NOT NULL 
      THEN ARRAY(SELECT jsonb_array_elements_text(NEW.raw_user_meta_data->'preferred_languages'))
      ELSE ARRAY['English']
    END,
    CASE 
      WHEN NEW.raw_user_meta_data->'occupation' IS NOT NULL 
      THEN ARRAY(SELECT jsonb_array_elements_text(NEW.raw_user_meta_data->'occupation'))
      ELSE NULL 
    END
  );
  RETURN NEW;
